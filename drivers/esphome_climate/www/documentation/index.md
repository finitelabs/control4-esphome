...
This driver provides specialized support for ESPHome devices with climate
entities, allowing them to be controlled through the Control4 thermostat proxy.
...
- Control4 OS 3.3+
- ESPHome driver configured and connected to an ESPHome device with climate
  entities

# <span style="color:#17BCF2">Features</span>

- Control4 ThermostatV2 Proxy integration for native Control4 thermostat control
- HVAC mode control (Off, Heat, Cool, Auto, Fan Only)
- Heat and cool setpoint management with automatic temperature unit conversion
- Fan mode control
- Hold mode / preset support (Home, Away, Eco, Sleep, etc.)
...
Climate driver to the climate entity exposed by the main driver.
...
#### Temperature Scale [ **_Fahrenheit_** | Celsius ]

Sets the temperature scale used for displaying and setting temperatures. Default
is `Fahrenheit`. When set to Fahrenheit, temperatures from ESPHome (which are
always in Celsius) are automatically converted.

## Connections

### Thermostat (provider)

The Control4 ThermostatV2 proxy connection. This is automatically managed by the
driver and provides the thermostat functionality to Control4.

### ESPHome Climate (consumer)

Bind this connection to the climate entity exposed by the main ESPHome driver.
