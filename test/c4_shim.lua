...
function C4:GetDeviceID() return 12345 end
function C4:GetDeviceData(deviceId, key)
  if key == "name" then return "Test Device" end
  return nil
end
...
function C4:SetTimer(delay_ms, callback, repeating)
  ...
  -- Create the handle first (this is what gets returned and passed to callback)
  local handle = {
    id = id,
  ...
  local timer = {
    ...
    cancelled = false,
    handle = handle  -- Store the handle for callback
  }
...
function C4:ProcessTimers()
  ...
  for id, timer in pairs(timers) do
    if not timer.cancelled and now >= timer.next_fire then
      -- Call callback with timer handle and skips count (0)
