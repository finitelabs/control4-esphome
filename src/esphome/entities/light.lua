...
function LightEntity:discovered(entity)
  ...
  local bindingId = assert(
    bindings:getOrAddDynamicBinding(self.TYPE, "light_" .. entity.ref, "PROXY", true, entity.name, "ESPHOME_LIGHT")
  ).bindingId
...
function LightEntity:updated(entity, state)
  ...
  local binding = bindings:getDynamicBinding(self.TYPE, "light_" .. entity.ref)
  if binding ~= nil then
